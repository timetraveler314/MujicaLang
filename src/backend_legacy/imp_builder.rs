...
pub struct ImpBuilder {
    // Tracks current variable assignments
    var_stack: Vec<(String, ImpVar)>,
...
impl ImpBuilder {
    ...
    pub fn resolve_var(&self, var: &str) -> Result<ImpVar, Error> {
        for (name, imp_var) in self.var_stack.iter().rev() {
            if name == var {
                return Ok(imp_var.clone());
            }
        }
        Err(Error::UnboundVariable(var.to_string()))
    }
    
    pub fn initialize_var(&mut self, anf_var: &str, imp_var: ImpVar) {
        if !self.var_stack.iter().any(|(name, _)| name == anf_var) {
            ...
            if should_initialize {
                self.emit(format!("{}* {} = malloc(sizeof({}));", imp_var.ty, imp_var.mangle(), imp_var.ty));
            } else {
                self.emit(format!("{} {}; // {}", imp_var.ty, imp_var.mangle(), anf_var));
            }
        }
    }

    /// Bind a new ANF variable to a C variable
    pub fn push_var(&mut self, anf_var: &str, imp_var: ImpVar) {
        self.var_stack.push((anf_var.to_string(), imp_var));
    }

    /// Pop the last variable binding
    pub fn pop_var(&mut self) -> Option<(String, ImpVar)> {
        self.var_stack.pop()
    }
